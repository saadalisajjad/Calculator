...
const ScientificPanel = ({ onAction, onDelete, panelState, onToggle }) => {
    ...
    return (
        <div className="flex h-full w-full overflow-hidden bg-[#00796b]">
            ...
            <div className="md:hidden flex flex-col w-10 h-full bg-[#00695c] border-r border-[#004d40]/40">

                <div
                    onClick={() => onToggle('LEFT')}
                    className="flex-1 flex items-center justify-center cursor-pointer active:bg-[#004d40] border-b border-[#004d40]/30"
                >
                    <span className="text-white text-2xl font-bold">‹</span>
                </div>
                <div
                    onClick={() => onToggle('RIGHT')}
                    className="flex-1 flex items-center justify-center cursor-pointer active:bg-[#004d40]"
                >
                    <span className="text-white text-2xl font-bold">›</span>
                </div>
            ...
            <div className="flex flex-col md:flex-row h-full w-full">
                ...
                <div className={`${panelState === 2 ? 'hidden md:grid' : 'grid'} grid-cols-2 w-full md:w-[60%] gap-0`}>
                    {advGroup1.map((f) => (
                        <Button key={f} label={f === 'sqrt' ? '√' : f === 'pi' ? 'π' : f} variant="adv1" onclick={() => onAction(f)}
                ...
                <div className={`${panelState === 1 ? 'hidden md:grid' : 'grid'} grid-cols-2 w-full md:w-[40%] gap-0 md:border-l border-gray-600/30`}>
                    {advGroup2.map((f) => (
                        <Button key={f} label={f} variant="adv2" onclick={() => onAction(f)}
